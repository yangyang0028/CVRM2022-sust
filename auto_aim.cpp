...
static int box_size(const cv::Rect &rect) {
...
int main(){
    ...
    IRuntime *runtime = createInferRuntime(gLogger); //创建运行时环境 IRuntime对象，传入 gLogger 用于打印信息
    assert(runtime != nullptr);
    ICudaEngine *engine = runtime->deserializeCudaEngine(trtModelStream, size);//将序列化得到的结果进行反序列化，以执行后续的inference
    ...
    void *buffers[2];
    const int inputIndex = engine->getBindingIndex(INPUT_BLOB_NAME); //获取输入节点索引
    const int outputIndex = engine->getBindingIndex(OUTPUT_BLOB_NAME); //获取输出节点索引
    assert(inputIndex == 0);
    ...
    while(true) {
        ...
        for (size_t i = 0; i < res.size(); i++) {
            ...
            if ((res[i].class_id == 0 || res[i].class_id == 1) && box_size(r) >= MIN_AIM_SIZE ) {
            ...
            switch ((int) (res[i].class_id)) {
                case 0: {
                    cv::putText(frame, label, cv::Point(r.x, r.y - 1), cv::FONT_HERSHEY_PLAIN, 1.2,
                                blue, 2);
                    break;
                }
                case 1: {
        ...
        int fps = (int) (1000.0 / MIN(std::chrono::duration_cast<std::chrono::milliseconds>(end - start).count(), 1));
        std::string video_fps = "FPS: " + std::to_string(fps);
        std::string aim_point_info = "Aim Point: " + getStringFromPoint(aim_point);
        cv::putText(frame, video_fps, cv::Point(10, 40), cv::FONT_HERSHEY_PLAIN, 3, cv::Scalar(255, 255, 255), 2,
                    cv::LINE_AA);
        cv::putText(frame, aim_point_info, cv::Point(10, 80), cv::FONT_HERSHEY_PLAIN, 3, cv::Scalar(255, 255, 255),
                    2,
                    cv::LINE_AA);
